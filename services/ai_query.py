...
class QueryParser:
    ...
    def parse_query(self, query: str) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ["what", "which", "show", "list"]):
            if any(
                word in query_lower
                for word in ["buy", "bought", "purchase", "food", "item"]
            ):
                return self._parse_item_query(query_lower)

        if any(
            word in query_lower
            for word in ["total", "spend", "spent", "cost", "expense"]
        ):
    ...
    def _query_items(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if params.get("specific_date"):
            receipts = self.db_service.get_receipts_by_date_range(
                params["specific_date"], params["specific_date"]
            )
            items = []
            ...
            formatted_results.append(
                {
                    ...
                    "receipt_date": datetime.fromisoformat(
                        result.metadata.get("receipt_date", "2024-01-01")
                    ).date(),
...
class ResponseFormatter:
    ...
    def format_response(
        self, query: str, results: List[Dict[str, Any]], parsed_query: Dict[str, Any]
    ) -> str:
    ...
    def _format_spending_response(
        ...
        if result["type"] == "daily_total":
            ...
            return (
                f"Your total food expenses over the last {days} days were ${total:.2f}."
            )
    ...
    def _format_stores_response(self, results: List[Dict[str, Any]], query: str) -> str:
        ...
        if not stores:
            time_phrase = (
                f"in the last {days_back} days" if days_back else "in your receipts"
            )
    ...
    def _format_no_results_response(
        self, query: str, parsed_query: Dict[str, Any]
    ) -> str:
...
class AIQueryService:
    ...
    def __init__(self):
        ...
        if not config.OPENROUTER_API_KEY:
            raise ValueError(
                "OpenRouter API key is required for AI query functionality"
            )
