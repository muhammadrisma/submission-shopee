...
class TextVectorizer:
    ...
    def fit(self, documents: List[str]):
        ...
        for word in self.vocabulary:
            ...
            self.idf_scores[word] = math.log(doc_count / (doc_freq + 1))  # +1 for smoothing
...
class CustomVectorDB:
    ...
    def _load_vectorizer_state(self) -> bool:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            cursor.execute("SELECT vocabulary, idf_scores FROM vectorizer_state WHERE id = 1")
            row = cursor.fetchone()
    ...
    def search_similar(self, query: str, top_k: int = 10, min_similarity: float = 0.1) -> List[VectorSearchResult]:
        """
        ...
        with self.db_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT item_id, item_name, vector_data, metadata FROM item_vectors")
    ...
    def find_similar_items(self, item_name: str, top_k: int = 5) -> List[VectorSearchResult]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            vocabulary_size = len(self.vectorizer.vocabulary) if self.vectorizer.is_fitted else 0
