...
from utils.error_handling import ErrorCategory, ErrorSeverity, error_handler
...
class FoodReceiptAnalyzerApp:
    ...
    def render_upload_page(self):
        ...
        if result:
            ...
            with col1:
                if st.button(
                    "🤖 Ask Questions About This Receipt", use_container_width=True
                ):
    ...
    def render_dashboard_page(self):
        ...
        try:
            ...
            if stats["receipt_count"] == 0:
                st.info(
                    "📝 No receipts uploaded yet. Upload your first receipt to see analytics!"
                )
                        ...
                        with col1:
                            ...
                            if receipt.items:
                                ...
                                for item in receipt.items[:3]:
                                    st.write(
                                        f"• {item.item_name} - ${item.total_price:.2f}"
                                    )
    ...
    def render_settings_page(self):
        ...
        with st.expander("🤖 AI Query Configuration"):
            ...
            if config.OPENROUTER_API_KEY:
                ...
                st.write(
                    "To enable AI queries, add your OpenRouter API key to the .env file:"
                )
        ...
        with st.expander("💾 Database Configuration"):
            ...
            try:
                ...
                st.write(
                    f"**Records:** {stats['receipt_count']} receipts, {stats['item_count']} items"
                )
            except Exception as e:
            ...
            for receipt in recent_receipts:
                with st.expander(
                    ...
                    with col1:
                        ...
                        if receipt.items:
                            items_preview = ", ".join(
                                [item.item_name for item in receipt.items[:3]]
                            )
                            if len(receipt.items) > 3:
                                items_preview += (
                                    f" and {len(receipt.items) - 3} more..."
                                )
    ...
    def _check_tesseract_status(self):
        ...
        try:
            ...
            if st.button("📖 View Installation Guide"):
                st.info(
                    "Check the INSTALLATION.md file for detailed installation instructions."
                )
    ...
    def run(self):
        ...
        try:
            ...
            with col2:
                st.caption(
                    "🧾 Food Receipt Analyzer - Powered by AI and Computer Vision"
                )
                st.caption(
                    f"Session ID: {st.session_state.get('session_id', 'unknown')[:8]}..."
                )
