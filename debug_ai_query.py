...
def debug_query_processing():
    ...
    if results:
        ...
        for period, start_date in [("last week", last_week), ("last month", last_month), ("last year", last_year)]:
            receipts_in_period = db_service.get_receipts_by_date_range(start_date, today)
            items_in_period = sum(len(r.items) for r in receipts_in_period)
            print(f"     {period}: {len(receipts_in_period)} receipts, {items_in_period} items")
