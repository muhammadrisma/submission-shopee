            ...
            if result['parsed_query']['intent'] == 'semantic_search' and result['results']:
                ...
                for i, res in enumerate(result['results'][:3]):
                    if 'similarity_score' in res:
                        print(f"     {i+1}. {res['item_name']}: {res['similarity_score']:.3f}")
    ...
    for test_case in test_cases:
        ...
        if results:
            ...
            for i, result in enumerate(results):
                print(f"     {i+1}. {result.item_name} (similarity: {result.similarity_score:.3f})")
                found_items.append(result.item_name)
            
            # Check accuracy
            expected_found = sum(1 for item in test_case['expected_items'] if item in found_items)
            accuracy = expected_found / len(test_case['expected_items']) if test_case['expected_items'] else 0
            print(f"   ✅ Accuracy: {accuracy:.1%} ({expected_found}/{len(test_case['expected_items'])} expected items found)")
        else:
