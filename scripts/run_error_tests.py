...
def test_retry_mechanism():
    ...
    try:
        ...
        if result == "success" and call_count == 3:
            ...
            print(
                f"❌ Successful retry test failed: result={result}, calls={call_count}"
            )
    except Exception as e:
...
def test_error_handler():
    ...
    try:
        ...
        if (
            not result["success"]
            and result["error"]["category"] == "validation"
            and "Fix the input" in result["error"]["recovery_suggestions"]
        ):
