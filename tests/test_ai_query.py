...
from services.ai_query import (
    OpenRouterClient, QueryParser, SQLQueryGenerator, 
    ResponseFormatter, AIQueryService, get_ai_query_service
)
...
class TestQueryParser(unittest.TestCase):
    ...
    def test_parse_item_query_yesterday(self):
        ...
        self.assertEqual(result['parameters']['specific_date'], date.today() - timedelta(days=1))
...
class TestSQLQueryGenerator(unittest.TestCase):
    ...
    def test_query_items_specific_date(self):
        ...
        mock_item = ReceiptItem(
            id=1, receipt_id=1, item_name="Pizza", quantity=1,
            unit_price=Decimal("12.99"), total_price=Decimal("12.99")
        )
        mock_receipt = Receipt(
            id=1, store_name="Pizza Place", receipt_date=date.today(),
            total_amount=Decimal("12.99"), upload_timestamp=datetime.now(),
            raw_text="", image_path="", items=[mock_item]
        )
    ...
    def test_query_spending_date_range(self):
        ...
        mock_receipts = [
            Receipt(
                id=1, store_name="Store A", receipt_date=date.today(),
                total_amount=Decimal("25.50"), upload_timestamp=datetime.now(),
                raw_text="", image_path="", items=[]
            ),
            Receipt(
                id=2, store_name="Store B", receipt_date=date.today(),
                total_amount=Decimal("15.75"), upload_timestamp=datetime.now(),
                raw_text="", image_path="", items=[]
            )
        ]
    ...
    def test_query_stores_with_item(self):
        ...
        self.mock_db_service.get_stores_with_item.assert_called_once_with('hamburger', 7)
...
class TestResponseFormatter(unittest.TestCase):
    ...
    def test_format_items_response_single_date(self):
        ...
        response = self.formatter.format_response("What did I buy today?", results, parsed_query)
    ...
    def test_format_spending_response_daily(self):
        ...
        response = self.formatter.format_response("How much did I spend today?", results, parsed_query)
    ...
    def test_format_stores_response_multiple(self):
        ...
        response = self.formatter.format_response("Where did I buy hamburger?", results, parsed_query)
    ...
    def test_format_general_response_with_ai(self, mock_general):
        ...
        response = self.formatter.format_response("General query", results, parsed_query)
...
class TestAIQueryService(unittest.TestCase):
    ...
    def setUp(self, mock_config):
        ...
        with patch('services.ai_query.OpenRouterClient'), \
    ...
    def test_process_query_success(self):
        ...
        self.service.response_formatter.format_response.return_value = formatted_response
