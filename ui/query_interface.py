...
from services.ai_query import AIQueryService, get_ai_query_service
...
class QueryInterface:
    ...
    def render_query_section(self):
        ...
        if not self.ai_service:
            ...
            st.info(
                "💡 Set your OPENROUTER_API_KEY in the .env file to enable natural language queries."
            )
            return
        ...
        if result["success"]:
            ...
            if result["parsed_query"]["intent"] == "semantic_search":
                if result["results"] and "similarity_score" in result["results"][0]:
                    ...
                    st.info(
                        f"🔍 **Vector Search Used** - Top similarity: {top_similarity:.1%}"
                    )
                else:
            ...
            with st.expander("📊 Query Details"):
                ...
                with col1:
                    ...
                    st.metric(
                        "Intent",
                        result["parsed_query"]["intent"].replace("_", " ").title(),
                    )
    ...
    def render_query_stats(self):
        ...
        with col3:
            if history:
                avg_time = sum(
                    item["result"]["execution_time"] for item in history
                ) / len(history)
        ...
        if len(history) > 0:
            ...
            if intent_counts:
                ...
                for intent, count in sorted(
                    ...
                    st.write(
                        f"**{intent.replace('_', ' ').title()}:** {count} queries ({percentage:.1f}%)"
                    )
