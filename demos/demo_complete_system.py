...
def demo_complete_pipeline():
    ...
    print(
        f"📊 Database: {stats['receipt_count']} receipts, {stats['item_count']} items"
    )
    ...
    for search_term, description in similarity_tests:
        ...
        if results:
            for i, result in enumerate(results, 1):
                print(
                    f"  {i}. {result.item_name} ({result.similarity_score:.1%} similar)"
                )
                print(
                    f"     ${result.metadata.get('price', 0):.2f} from {result.metadata.get('store_name', 'Unknown')}"
                )
        else:
