...
def main():
    ...
    for item in items:
        print(
            f"   - {item.item_name}: {item.quantity} x ${item.unit_price} = ${item.total_price}"
        )
    print()
    ...
    for result in apple_results:
        print(
            f"   - {result['item_name']} at {result['store_name']} on {result['receipt_date']}"
        )
    print()
    ...
    for result in all_apples:
        print(
            f"   - {result['item_name']} at {result['store_name']} (${result['total_price']})"
        )
    print()
